...
################################################
##Mapping Details##############################
MapName="FileToDBMapping"
srcfile="$infuser/INPUT/sales_data"
tgtfile="$infuser/OUTPUT/testdb.csv"
tgtopr="Insert"
tgtcodpg="UTF"
tgtfmt="FLAT"
MapTask="FILETODBMAPTASK"
TaskMapPath=$1
#####################################################
###Expression Details#################################
expr1="Tot_Sales_Amt"
expr2="Target"
expr3="exp_FormatOrderDate"
tgtfields="Formatted_Order_Date Tot_Sales_Amt Order_ID Customer_Name Product Quantity Price Order_Date"
########################################################
# Define the patterns to check
quantity_price_pattern="Quantity.*\*.*Price"  # Pattern to match Quantity * Price
date_format_pattern="MM/DD/YYYY"             # Pattern to match MM/DD/YYYY
#########################################################
# Initialize flags
quantity_price_found=false
date_format_found=false
##########################################################
####Expression Stage Names################################
valid_stages=("exclude_quantity" "expression" "exp_formatorderdate")
###############################################################################################
...
#sessionId="2HuSa74nPk7goJLEEfmiOP"
...
response3=$(curl -s -X GET "$serviceUrl/api/v2/mapping/name/$MapName/" -H "Content-Type: application/xml" \
	-H "icSessionId: $sessionId")
tgtobj=$(echo $response3 | jq -r '[.deployTime, (.parameters[] | select(.type == "TARGET") | .targetObject, .operationType, .dataFormat.dataFormatAttributes.codePage, .dataFormat.formatId)] | @tsv')
echo $tgtobj
srcobj=$(echo $response3 | jq -r '.parameters[] | select(.type == "EXTENDED_SOURCE") | .extendedObject.objects[0] | .name | select (.!= null)')
echo $srcobj 
...
if [ $stgPass -eq 1 ]; then 
# Parse the output
output=$(cat bin/@3.bin | jq -r '.content.transformations[]? .fields[]? | select(.expression != null) | "\(.name): \(.expression)"')
...
while IFS= read -r line; do
  ...
  if [[ $line =~ $quantity_price_pattern ]]; then
    echo "Found Quantity * Price expression in: $line"
    quantity_price_found=true
  fi

  # Check for MM/DD/YYYY pattern
  if [[ $line =~ $date_format_pattern ]]; then
    echo "Found date format MM/DD/YYYY in: $line"
    date_format_found=true
  fi
done <<< "$output"

# Final checks and setting values
if $quantity_price_found && $date_format_found; then
  echo "Both Quantity * Price and MM/DD/YYYY patterns were found."
  exprStatus="Success"
  exprFeedback="Expression for sales_amt & formatted_order_Date in the Informatica cloud"
  exprObserv="Expression for sales_amt & formatted_order_Date are created in the Informatica cloud as given in the description"
  exprScore=20
  exprPass=1

elif $quantity_price_found; then
  echo "Only Quantity * Price pattern was found."
  exprStatus="Success"
  exprFeedback="Expression for sales_amt in the Informatica cloud"
  exprObserv="Expression for sales_amt is created in the Informatica cloud as given in the description"
  exprScore=10
  exprPass=1

elif $date_format_found; then
  echo "Only MM/DD/YYYY pattern was found."
  exprStatus="Success"
  exprFeedback="Expression for formatted_order_Date in the Informatica cloud"
  exprObserv="Expression for formatted_order_Date is created in the Informatica cloud as given in the description"
  exprScore=10
  exprPass=1

else
  echo "Neither pattern was found."
  exprStatus="Failure"
  exprFeedback="Expression for sales_amt & formatted_order_Date not created in the Informatica cloud"
  exprObserv="Expression for sales_amt & formatted_order_Date are not created in the Informatica cloud as given in the description"
  exprScore=0
  exprPass=0
fi

# Update Total
Total=$((Total + exprScore))
else
  echo "Neither pattern was found."
  exprStatus="Failure";
  exprFeedback="Expression for sales_amt & formated_order_Date not created in the Informatica cloud";
  exprObserv="Expression for sales_amt & formated_order_Date are not created in the Informatica cloud as given in the description";
  exprScore=0;
  exprPass=0;
  Total=`expr $Total + $exprScore`;
...
##############Fields check#######
#########################################################################################################################
if [ $exprPass -eq 1 ]; then 
arrvar=$(cat bin/@3.bin | jq -r '.content.annotations[] | select(.jsonBlob != null) | (.jsonBlob | fromjson | keys | .[])')
for value in ${arrvar[@]}; do
  if [[ "$value" == "Target" ]]; then
  fields=$(cat bin/@3.bin | jq -r --arg name "$value" '.content.transformations[] | select(.name == $name) | .fields[]?.name')
  echo $fields
  fi
  # Initialize score
done
...
if [ $exprPass -eq 1 ]; then 
filvar=$(cat bin/@3.bin | jq -r '.content.transformations[] | .filterConditions[]? | "\(.fieldName) \(.operator) \(.filterValue)"' | grep -wic "quantity")
if [ $filvar -ge 1 ]; then 
  filStatus="Success";
  filFeedback="filter is created in the Informatica cloud";
  filObserv="Filter  are created in the Informatica cloud as given in the description";
  filScore=10;
  filPass=1;
  Total=`expr $Total + $filScore`;
else
  filStatus="Failure";
  filFeedback="Filter is not created in the Informatica cloud";
  filObserv="Filter is  not created in the Informatica cloud as given in the description";
  filScore=0;
  filPass=0;
  Total=`expr $Total + $filScore`;
fi
...
echo $failtgtrows
echo $errmsg
if [ $succsrcrows -ge 2 ] && [ $succtgtrows -ge 2 ]; then 
  logStatus="Success";
...
buckcnt=$(AWS_ACCESS_KEY_ID=$aws_acckey AWS_SECRET_ACCESS_KEY=$aws_seckey aws s3 ls s3://$tgtbuckname/$infuser/ | grep -wic $awstgtfile)
if [ $buckcnt -ge 1 ]; then 
extractfile=$(AWS_ACCESS_KEY_ID=$aws_acckey AWS_SECRET_ACCESS_KEY=$aws_seckey aws s3 cp s3://$tgtbuckname/$infuser/$awstgtfile .)
cd $1
...
Result='@responsestart@ \n
    ...
    "TestCases": [
		...
		{
            "Name": "Filter checked in the stages",
            "Status": "'$filStatus'",
            "Skill": "Beginner",
            "Score": "'$filScore'%",
            "Feedback": "'$filFeedback'",
            "Observation": "'$filObserv'",
            "ConsoleOutput": ""
        },
		 {
