...
awstgtfile="HDCFRunningTotal.csv"
exptfile="HDCFRunningTotal_exp.csv"
...
################################################
##Mapping Details##############################
MapName="HDCFRunningTotal_map"
srcfile="$infuser/INPUT/SrcEmpDetails.json"
tgtfile="$infuser/OUTPUT/HDCFRunningTotal.csv"
tgtopr="Insert"
tgtcodpg="UTF"
tgtfmt="FLAT"
MapTask="HDCFRunningTotal_task"
TaskMapPath=$1
#####################################################
###Expression Details#################################
expr1="expression"
expr2="target"
tgtfields="SALARY_RUNNING ID SALARY"
########################################################
# Define the patterns to check
sal_expv="V_SALARY\+SALARY,SALARY"
...
while IFS= read -r line; do
  ...
  # Check for pattern1 (V_SALARY+SALARY,SALARY)
  if [[ $line =~ $sal_expv ]]; then
    echo "Found pattern1: $sal_expv"
    sal_expv_found=true
  fi

  # Check for pattern2 (V_SALARY=SALARY,SALARY,V_SALARY)
  if [[ $line =~ $sal_expr ]]; then
    echo "Found pattern2: $sal_expr"
    sal_expr_found=true
  fi
done <<< "$cl_output"

echo $sal_expv_found $sal_expr_found "sal_expv_found"
# Final checks and setting values
if $sal_expv_found && $sal_expr_found; then
  echo "The expressions are valid."
  exprStatus="Success"
  exprFeedback="Expression for running salary  in the Informatica cloud"
  exprObserv="Expression for running salary  are created in the Informatica cloud as given in the description"
  exprScore=25
  exprPass=1

elif $sal_expv_found; then
  exprStatus="Success"
  exprFeedback="Expression for v_salary in the Informatica cloud"
  exprObserv="Expression for v_salary is created in the Informatica cloud as given in the description"
  exprScore=13
  exprPass=1

elif $sal_expr_found; then
  exprStatus="Success"
  exprFeedback="Expression for running_salary in the Informatica cloud"
  exprObserv="Expression for running_salary is created in the Informatica cloud as given in the description"
  exprScore=12
  exprPass=1
  ...
  exprStatus="Failure"
  exprFeedback="Expression for v_salary & salary_running not created in the Informatica cloud"
  exprObserv="Expression for v_salary & salary_running are not created in the Informatica cloud as given in the description"
  exprScore=0
  exprPass=0
fi
