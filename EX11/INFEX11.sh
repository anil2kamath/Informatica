...
awsRemainingtgtfile="RemainingOrders.csv"
exptRemainingfile="RemainingOrders_exp.csv"
awsCaliforniatgtfile="CaliforniaOrders.csv"
expCaliforniafile="CaliforniaOrders_exp.csv"
awsFloridatgtfile="FloridaOrders.csv"
expFloridafile="FloridaOrders_exp.csv"
...
################################################
##Mapping Details##############################
MapName="OrderRouter_map"
srcfile="$infuser/INPUT/OrderDetails.json"
tgtRemfile="$infuser/OUTPUT/RemainingOrders.csv"
tgtCalfile="$infuser/OUTPUT/CaliforniaOrders.csv"
tgtFlofile="$infuser/OUTPUT/FloridaOrders.csv"
tgtopr="Insert"
tgtcodpg="UTF"
tgtfmt="FLAT"
MapTask="OrderRouter_task"
TaskMapPath=$1
#####################################################
###Expression Details#################################
expr1="router"
expr2="remainingorders"
tgtfields="billdate itemname quantity state customertype"
########################################################
# Define the patterns to check
floridafilter="State=Florida"
californiafilter="State=California"
#sal_expr="V_SALARY=SALARY,SALARY,V_SALARY"
#########################################################
# Initialize flags
sal_expv_found=false
#sal_expr_found=false
##########################################################
####Expression Stage Names################################
valid_stages=("router", "remainingorders")
###############################################################################################
...
tgtRemfname=$(echo $tgtobj | grep -ic "$tgtRemfile")
tgtCalfname=$(echo $tgtobj | grep -ic "$tgtCalfile")
tgtFlofname=$(echo $tgtobj | grep -ic "$tgtFlofile")
...
if ([ $srcfname -ge 1 ] && [ $tgtRemfname -ge 1 ] && [ $tgtCalfname -ge 1 ] && [ $tgtFlofname -ge 1 ] && [ $tgtoper -ge 1 ] && [ $tgtcod -ge 1 ] && [ $tgtfmt -ge 1 ]); then 
  srcfStatus="Success";
...
if [ $stgPass -eq 1 ]; then 
# Parse the output
floridaoutput=$(cat bin/@3.bin | jq -r '.content.transformations[]? .groupFilterConditions[]? .simpleFilterConditions[]? | "\(.fieldName)\(.operator)\(.filterValue)"' | grep -wic "state=florida")
californiaoutput=$(cat bin/@3.bin | jq -r '.content.transformations[]? .groupFilterConditions[]? .simpleFilterConditions[]? | "\(.fieldName)\(.operator)\(.filterValue)"' | grep -wic "state=california")


if [ $floridaoutput -ge 1 ] && [ $californiaoutput -ge 1 ]; then
  echo "The expressions are valid."
  exprStatus="Success"
  exprFeedback="Filters for Router found in the Informatica cloud"
  exprObserv="Filters are created in the Informatica cloud as given in the description"
  exprScore=15
  exprPass=1
  ...
  exprStatus="Failure";
  exprFeedback="Not all filters were found or created in the Informatica cloud";
  exprObserv="Not all the filters were found or  created in the Informatica cloud as given in the description";
  exprScore=0;
  exprPass=0;
...
echo $failtgtrows
echo $errmsg
if [ $succsrcrows -ge 100 ] && [ $succtgtrows -ge 100 ]; then 
  logStatus="Success";
...
buckcnt=$(AWS_ACCESS_KEY_ID=$aws_acckey AWS_SECRET_ACCESS_KEY=$aws_seckey aws s3 ls s3://$tgtbuckname/$infuser/OUTPUT/ | grep -wic $awsRemainingtgtfile)
if [ $buckcnt -ge 1 ]; then 
AWS_ACCESS_KEY_ID=$aws_acckey AWS_SECRET_ACCESS_KEY=$aws_seckey aws s3 cp s3://$tgtbuckname/$infuser/OUTPUT/$awsRemainingtgtfile $1
extractfile=$(ls $awsRemainingtgtfile)
echo $extractfile
ls -lart $awsRemainingtgtfile
diff_result1=$(diff "$exptRemainingfile" sort  "$extractfile" sort)
