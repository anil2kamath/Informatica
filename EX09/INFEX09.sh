...
awstgtfile="EmpDep.csv"
exptfile="EmpDep_exp.csv"
...
################################################
##Mapping Details##############################
MapName="EmpDepJoiner_map"
srcfile="$infuser/INPUT/Department_Data.csv"
tgtfile="$infuser/OUTPUT/EmpDep.csv"
tgtopr="Insert"
tgtcodpg="UTF"
tgtfmt="FLAT"
MapTask="EmpDepJoiner_task"
TaskMapPath=$1
#####################################################
###Expression Details#################################
expr1="joiner"
expr2="Department"
tgtfields="DEPTNAME EMPNO FIRSTNAME LOCATION LASTNAME"
########################################################
...
valid_stages=("joiner" "department")
###############################################################################################
...
if [ $stgPass -eq 1 ]; then 
# Parse the output
output=$(cat bin/@3.bin | jq -r '.content.transformations[]? .joinConditions[]? | "\(.leftOperand) \(.operator) \(.rightOperand)"')
echo $output "output"
