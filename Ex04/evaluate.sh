...
awstgtfile="EmpDepLookUp.csv"
exptfile="EmpDepLookUp_exp.csv"
...
################################################
##Mapping Details##############################
MapName="EmpDepLkUp_map"
srcfile="$infuser/INPUT/Employee_Data.csv"
tgtfile="$infuser/OUTPUT/EmpDepLookUp.csv"
tgtopr="Insert"
tgtcodpg="UTF"
tgtfmt="FLAT"
MapTask="EmpDepLkUp_task"
TaskMapPath=$1
#####################################################
###Expression Details#################################
expr1="department"
#expr2="manufactureryearlysales"
tgtfields="DEPTNAME LOCATION EMPNO FIRSTNAME"
########################################################
...
output=$(cat bin/@3.bin | jq -r '.content.transformations[]?.lookupConditions[]? | "\(.leftOperand)\(.operator)\(.rightOperand)"')
echo $output "output"
cl_output=$(echo "$output" | tr -d ' ' | grep -wic $sal_expv)
echo $cl_output "cl_output"
echo $cl_output
# Process each line of the output
if [ $cl_output -ge 1 ]; then
  echo "The expressions are valid."
  exprStatus="Success"
  exprFeedback="TotalSalary as SUM of salary found in the Informatica cloud"
  exprObserv="Aggregate for SUM(SALARY) is created in the Informatica cloud as given in the description"
  exprScore=15
  exprPass=1
  ...
  exprStatus="Failure";
  exprFeedback="TotalSalary as SUM(SALARY) was not found or not created in the Informatica cloud";
  exprObserv="Aggregate for SUM(SALARY) not created in the Informatica cloud as given in the description";
  exprScore=0;
  exprPass=0;
