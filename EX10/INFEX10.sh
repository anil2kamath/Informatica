...
awstgtfile="CompanyEmployees.xml"
exptfile="CompanyEmployees_exp.xml"
...
################################################
##Mapping Details##############################
MapName="CompanyEmployeeHierarchy_map"
srcfile="$infuser/INPUT/CompanyEmployee.txt"
tgtfile="$infuser/OUTPUT/CompanyEmployees.xml"
tgtopr="Insert"
tgtcodpg="UTF"
tgtfmt="FLAT"
MapTask="CompanyEmployeeHierarchy_task"
TaskMapPath=$1
#####################################################
###Expression Details#################################
expr1="hierarchybuilder"
expr2="companyemployee"
tgtfields="Output"
########################################################
# Define the patterns to check
#sal_expv="DEPTNO=workDept"
#sal_expr="V_SALARY=SALARY,SALARY,V_SALARY"
#########################################################
# Initialize flags
pk_attribute="companyname=true"
fk_attribute="name=true"
#sal_expr_found=false
##########################################################
####Expression Stage Names################################
valid_stages=("hierarchybuilder")
###############################################################################################
...
if [ $stgPass -eq 1 ]; then 
# Parse the output
pkoutput=$(cat bin/@3.bin | jq -r '.content.transformations[]? .xFieldMap.mappings[]? .xfields[]? |"\(.name)=\(.primaryKey)"' | grep -wic $pk_attribute)
fkoutput=$(cat bin/@3.bin | jq -r '.content.transformations[]? .xFieldMap.mappings[]? .xfields[]? |"\(.name)=\(.foreignKey)"' | grep -wic $fk_attribute)

echo $pkoutput "output primaryKey"
echo $fkoutput "output foreignKey"
cl_output=$((pkoutput + fkoutput))
echo $cl_output "cl_output"
echo $cl_output
# Process each line of the output
if [ $cl_output -eq 2 ]; then
  echo "The expressions are valid."
  exprStatus="Success"
  exprFeedback="Primary and Foreign key configured in Field Mapping of Hierarchy Builder"
  exprObserv="Primary and Foreign key configured in Field Mapping of Hierarchy Builder in the Informatica for CompanyName and Name attributes"
  exprScore=25
  exprPass=1
  ...
  exprStatus="Failure";
  exprFeedback="Primary and Foreign key NOT configured in Field Mapping of Hierarchy Builder as expected";
  exprObserv="Either Primary or Foreign key NOT configured in Field Mapping of Hierarchy Builder for CompanyName or Name attribute or both of the attributes";
  exprScore=0;
  exprPass=0;
